...
const AllStyles = () => {
  ...
  const addToCart = (productId: number) => {
    const product = allStylesProducts.find(p => p.id === productId);
    if (product) {
  ...
  const selectedProduct = checkoutModal.productId 
    ? allStylesProducts.find(p => p.id === checkoutModal.productId)
    : null;
      ...
      <header className="bg-deep-emerald shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <h1 className="text-2xl font-playfair font-bold text-soft-cream">All Styles</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-charcoal-black/70">
            Gender-neutral and inclusive fashion for everyone
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {allStylesProducts.map((product) => (
